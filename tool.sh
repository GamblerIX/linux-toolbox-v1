...
set -euo pipefail  # 严格模式：遇到错误就退出，未定义变量报错，管道错误传播
...
readonly GITHUB_RAW_URL="https://raw.githubusercontent.com/GamblerIX/linux-toolbox/main/tool.sh"
...
log() {
    ...
    case "$level" in
        ...
        "ERROR") echo -e "${ROOT}[ERROR]${NC} $message" >&2 ;;
...
EOF
        ...
        "debian")
            ...
            # Debian 12+ 需要 non-free-firmware
...
install_toolbox() {
    ...
    # 安装脚本
    if cp "$temp_file" /usr/local/bin/tool && chmod +x /usr/local/bin/tool; then
        # 确保目录存在
        mkdir -p "$TOOLBOX_DIR"
...
disable_bbr() {
    ...
    sed -i '/# BBR网络优化配置/d' /etc/sysctl.conf
